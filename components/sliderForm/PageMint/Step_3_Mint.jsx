    ...
    <div ref={(el) => (stepsRef.current[currentStep] = el)} className={styles.form__step}>
      <div className="pr-6 pl-2" ref={containerElement}>
        ...
        <div className="mb-6 items-center flex flex-col">
          ...
          <div
            className={`dark:bg-jacarta-700 dark:border-jacarta-600 border-jacarta-100 group relative flex max-w-md flex-col items-center justify-center rounded-lg border-2 border-dashed bg-white  w-[${widthRatioImage}px] h-[${heightRatioImage}px] text-center ${
              !previewImage ? "px-2 py-8" : "py-1 px-1"
            }`}
            style={{ width: `${widthRatioImage}px`, height: `${heightRatioImage}px` }}
          >
            <div className={`relative z-10 cursor-pointer  ${!previewImage ? "px-2 py-2" : "px-0 h-full w-full"}`}>
              {!previewImage ? (
                <div>
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" className="fill-jacarta-500 mb-2 inline-block dark:fill-white">
                ...
                <div className={`flex justify-center w-[${widthRatioImage}px] h-[${heightRatioImage}px] `} style={{ width: `${widthRatioImage}px`, height: `${heightRatioImage}px` }}>
                  <Image src={previewImage ? previewImage : "/"} fill={true} alt="Preview" className="object-contain h-full" />
                </div>
              )}
            </div>
          ...
          {file && <p className="text-warning text-2xs mt-3">⚠️ This is how your ad will be displayed on the media plateform. Be carefull to fill all the dotted square.</p> }
        </div>
      </div>
    </div>
  );
