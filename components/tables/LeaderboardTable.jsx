                ...
                <table className="w-full text-left">
                    ...
                    <tr>
                        {columns.map((col, index) => (
                            <th key={index} className="text-sm font-semibold text-jacarta-400 dark:text-jacarta-300">
                                {col.header}
                            </th>
                        ))}
                    </tr>
...
const LeaderboardTable = ({ activity }) => {
    ...
    const holderColumns = [
        ...
        { header: "Total Amount Spent", render: (item) => (
                <>
                    {item.totalSpent} USDC
                    <ul>
                        {item.details.map((detail, i) => (
                            <li key={i}>{detail.currency}: {detail.totalSpent}</li>
                        ))}
                    </ul>
                </>
            ) },
        { header: "Address", render: (item) => (
    ...
    const rewardedColumns = [
        { header: "Rank", render: (item) => item.rank },
        { header: "Total Amount Received", render: (item) => (
                <>
                    {item.totalReceived} USDC
                    <ul>
                        {item.details.map((detail, i) => (
                            <li key={i}>{detail.currency}: {detail.totalReceived}</li>
                        ))}
                    </ul>
                </>
            ) },
