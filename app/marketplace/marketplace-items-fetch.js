...
export const fetchOffer = async (nftContracts, tokenIds, chainId) => {
  ...
  const GET_TOKEN_DATA = gql`
    query ($nftContracts: [Bytes!], $tokenIds: [BigInt!]) {
      updateOffers(
  ...
  try {
    ...
    updateOffers.forEach((offer) => {
      ...
      if (
        !groupedOffers[nftContract] ||
        blockTimestamp > groupedOffers[nftContract].blockTimestamp
      ) {
    ...
    const latestOffers = updateOffers.map(
      (offer) => groupedOffers[offer.nftContract]
    );
