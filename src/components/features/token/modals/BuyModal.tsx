...
const BuyModal = ({
  ...
  useEffect(() => {
    ...
    if (nativeTokenBalance && nativeTokenBalance.value < amountInEthWithSlippageBN.toBigInt()) {
    ...
    if (currencyBalance && currencyBalance.value < priceBN.toBigInt()) {
                    ...
                    <>
                      {address ? (
                        <NormalButton
                          onClick={async () => {
                            try {
                              await switchChain(chainConfig.chainObject);
                            } catch (error) {
